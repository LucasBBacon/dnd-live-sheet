...
type ExistingReferenceResolver = (
  kind: RollbackKind,
  id: string,
...
const getRollbackRows = async (
  rollbackRunId: string,
): Promise<RollbackRowRow[]> =>
  db
    .select()
    .from(rollbackRows)
...
const getAppliedSourceRows = async (
  sourceRunId: string,
  ...
  db
    ...
    .from(importRows)
    .where(
      and(eq(importRows.runId, sourceRunId), eq(importRows.status, "applied")),
    )
  ...
  for (const sourceRow of sourceRows) {
    try {
      ...
      issues.push({
        ...
        message:
          error instanceof Error ? error.message : "Rollback row parse failed.",
        details: {
          sourceRowIndex: sourceRow.rowIndex,
          kind: sourceRow.kind,
          op: sourceRow.op,
...
const markRollbackIssues = async (
  rollbackRunId: string,
  issues: RollbackIssue[],
): Promise<void> => {
  await db
    .delete(rollbackIssues)
...
const updateRollbackRowStatus = async (
  ...
  await db
    ...
    .where(
      and(
        eq(rollbackRows.runId, rollbackRunId),
        eq(rollbackRows.rowIndex, rowIndex),
      ),
    );
};
    ...
    switch (entry.kind) {
      ...
      default:
        throw new RollbackPipelineError(
          `Unsupported rollback archive kind: ${entry.kind}.`,
        );
    }
  }
...
const applyRollbackRelation = async (
  relation: ImportRelationEntry,
  sourceRun: ImportRunRow,
): Promise<void> => {
  ...
  throw new RollbackPipelineError(
    `Unsupported rollback relation op: ${relation.op}.`,
  );
};
...
export const planRollbackRun = async ({
  ...
  if (sourceRun.status !== "applied") {
    throw new RollbackPipelineError(
      "Only applied import runs can be planned for rollback.",
    );
  }
  ...
  if (built.plannedRows.length > 0) {
    const rowsToInsert: Array<typeof rollbackRows.$inferInsert> =
      built.plannedRows.map((row, rowIndex) => {
        const status: "failed" | "planned" = failingRowIndexes.has(rowIndex)
          ? "failed"
          : "planned";
    ...
    await db.insert(rollbackRows).values(rowsToInsert);
  }
  ...
  if (hasErrors) {
    throw new RollbackPipelineError(
      "Rollback planning detected blocking issues.",
    );
  }
...
export const applyRollbackRun = async (
  ...
  if (run.status !== "planned") {
    throw new RollbackPipelineError(
      "Rollback run must be planned before apply.",
    );
  }
    ...
    await markRollbackIssues(rollbackRunId, [
      {
        ...
        message:
          error instanceof Error ? error.message : "Rollback apply failed.",
      },
    ]);
